...
export default {
    ...
    data() {
        return {
            services: null,
            form: useForm({
                server: this.current_request.responder,
                date: this.current_request.date['persian'],
                requester_comment: this.current_request.requester_comment,
                srvs: this.current_request.services_ids,
            })
        }
    },
    mounted () {
    ...
    methods: {
        ...
        submit() {
            if (this.form.srvs.length > 0)
                this.form.put('/customer/requests/' + this.current_request.id);
            else
...
<template>
    ...
    <header class="flex justify-between mb-10">
        <h2 class="text-xl md:text-2xl text-slate-700">ویرایش درخواست رزرو</h2>
